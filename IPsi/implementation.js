    ...
    if (equal(Term_dom_S_inner3,Term.ZERO)){ //3.1
      ...
      if (lessThan(Term_dom_S_inner3,S)) return "ψ("+S.inner1+","+S.inner2+","+fund(S.inner3,T)+")"; //3.3.1
        ...
        if (equal(dom(e),Term.ONE)){ //3.3.2.1
          ...
          var Term_fund_S_fund_T_0=null;
          if (isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.inner1,S.inner1)&&equal(Term_fund_S_fund_T_0.inner2,S.inner2)) return "ψ("+S.inner1+","+S.inner2+","+fund(S.inner3,"ψ("+fund(d,Term.ZERO)+","+Term_fund_S_fund_T_0.inner3+",0)")+")"; //3.3.2.2.1
 ...
 */
function limitOrd(n){
  return "ψ(0,0,"+(n==0?"0":"ψ(".repeat(n)+"0"+",0,0)".repeat(n))+")";
}
/**
 * @param {number} n 
 * @returns {number}
