...
function dom(S){
  ...
  if (S instanceof ThetaTerm){ //3
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)){ //3.1
      ...
      if (equal(Term_dom_S_sub,Term.ZERO)) return Term.SMALLOMEGA+""; //3.1.1
      else if (equal(Term_dom_S_sub,Term.ONE)) return S+""; //3.1.2
...
function fund(S,T){
  ...
  if (S instanceof ThetaTerm){ //3
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)){ //3.1
      var dom_S_sub=dom(S.sub);
      var Term_dom_S_sub=new Term(dom_S_sub);
      if (equal(Term_dom_S_sub,Term.ZERO)){ //3.1.1
        if (equal(dom(T),Term.ONE)) return "ψ("+fund(S,fund(T,Term.ZERO))+",0)"; //3.1.1.1
        return "ψ(0,0)"; //3.1.1.2
      }else if (equal(Term_dom_S_sub,Term.ONE)) return T+""; //3.1.2
      else return "θ("+fund(S.sub,T)+","+S.inner+")"; //3.1.3
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)){ //4.1
      ...
      if (equal(Term_dom_S_sub,Term.ZERO)||equal(Term_dom_S_sub,Term.ONE)) return T+""; //4.1.1
        ...
        if (lessThan(Term_dom_S_sub,S)) return "ψ("+fund(S.sub,T)+","+S.inner+")"; //4.1.2.1
          ...
          var c=Term_dom_S_inner.sub;
          var Term_fund_S_fund_T_0=null;
          if (equal(dom(T),Term.ONE)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.inner,S.inner)) return "ψ("+fund(S.sub,"θ("+fund(c,Term.ZERO)+","+Term_fund_S_fund_T_0.sub+")")+","+S.inner+")"; //4.1.2.2.1
          else return "ψ("+fund(S.sub,"θ("+fund(c,Term.ZERO)+",0)")+","+S.inner+")"; //4.1.2.2.2
        }
      }
      ...
      if (lessThan(Term_dom_S_inner,S)) return "ψ("+S.sub+","+fund(S.inner,T)+")"; //4.3.1
        ...
        if (Term_dom_S_inner instanceof PsiTerm&&equal(Term_dom_S_inner.inner,Term.ZERO)){ //4.3.2.1
          ...
          var c=Term_dom_S_inner.sub;
          var Term_fund_S_fund_T_0=null;
          if (equal(dom(T),Term.ONE)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"θ("+fund(c,Term.ZERO)+","+Term_fund_S_fund_T_0.inner+")")+")"; //4.3.2.2.1
          else return "ψ("+S.sub+","+fund(S.inner,"θ("+fund(c,Term.ZERO)+",0)")+")"; //4.3.2.2.2
        }
      }
    }
  }
