  ...
  while (scanner.hasNext()&&state!=EXIT){
    ...
    if (nums.indexOf(next)!=-1){
      ...
      if (nextnext=="_"){
        ...
        var nextnext=scanner.next();
        if (nextnext!=")") throw Error("Expected closing ) at position "+(scanner.pos-1)+", instead got "+nextnext+" in expression "+scanner.s);
        appendToRSum(PsiTerm.buildNoClone(superscriptterm,ZeroTerm.build(),innerterm));
    ...
    if (state==CLOSEDTERM){
      ...
      if (context==BRACES&&peek=="}"){
        state=EXIT;
      }else if (context==PSITERMSUPERSCRIPT&&(peek=="_"||peek=="(")){
        state=EXIT;
...
PsiTerm.build=function (sup,sub,inner){
  var r=new PsiTerm();
  r.sup=new Term(sup);
  r.sub=new Term(sub);
 ...
 * @returns {PsiTerm}
 */
PsiTerm.buildNoClone=function (sup,sub,inner){
  var r=new PsiTerm();
  r.sup=sup;
  r.sub=sub;
  r.inner=inner;
...
PsiTerm.prototype.clone=function (){
  return PsiTerm.build(this.sup,this.sub,this.inner);
}
/** @param {boolean} abbreviate */
PsiTerm.prototype.toString=function (abbreviate){
  ...
  else if (abbreviate&&this.sub.equal(Term.ZERO)) return "ψ^"+this.sup.toStringWithImplicitBrace(abbreviate)+"("+this.inner.toString(abbreviate)+")";
  else return "ψ^"+this.sup.toStringWithImplicitBrace(abbreviate)+"_"+this.sub.toStringWithImplicitBrace(abbreviate)+"("+this.inner.toString(abbreviate)+")";
}
PsiTerm.prototype.equal=function (other){
  if (!(other instanceof Term)) other=new Term(other);
  return other instanceof PsiTerm&&this.sup.equal(other.sup)&&this.sub.equal(other.sub)&&this.inner.equal(other.inner);
}
...
Term.ONE=new Term("ψ^0_0(0)");
Term.SMALLOMEGA=new Term("ψ^0_0(1)");
...
function inT(t){
  ...
  if (t instanceof PsiTerm) return inT(t.sup)&&inT(t.sub)&&inT(t.inner);
...
function inRT(t){
  ...
  if (t instanceof ZeroTerm) return true;
  if (t instanceof PsiTerm) return isNat(t.sup)&&isNat(t.sub)&&inRT(t.inner);
...
function dom(S){
  ...
  if (S instanceof PsiTerm){ //3
    ...
    if (equal(Term_dom_c,"0")){ //3.1
      if (b==0) return normalizeAbbreviations("1"); //3.1.1
      else return S+""; //3.1.2
    }else if (equal(Term_dom_c,"1")) return normalizeAbbreviations("ω"); //3.2
    else{ //3.3
      if (lessThan(Term_dom_c,S)) return dom_c; //3.3.1
      else{ //3.3.2
        var rank_c=rank(c);
        if (rank_c==0) return normalizeAbbreviations("ω"); //3.3.2.1
        else return S+""; //3.3.2.2
      }
    }
  }
...
function rank(S){
  ...
  if (!inRT(S)) throw Error("Invalid argument: "+S);
  if (S instanceof ZeroTerm) return 0; //1
  if (S instanceof SumTerm) return rank(S.getRight()); //2
  if (S instanceof PsiTerm){ //3
    var a=toNat(S.sup);
    var b=toNat(S.sub);
    var c=S.inner;
    ...
    var Term_dom_c=new Term(dom_c);
    if (equal(Term_dom_c,"0")){ //3.1
      if (b==0) return 0; //3.1.1
      else return a; //3.1.2
    }else if (equal(Term_dom_c,"1")) return 0; //3.2
    else{ //3.3
      if (lessThan(Term_dom_c,S)) return rank(c); //3.3.1
      else{ //3.3.2
        var rank_c=rank(c);
        if (rank_c==0) return 0; //3.3.2.1
        else return rank_c-1; //3.3.2.2
      }
    }
  }
  throw Error("No rule to compute rank of "+S);
}
/**
 * @param {Term|string} S 
 * @returns {string}
...
function cp(S){
  ...
  throw Error("No rule to compute CP of "+S);
}
/**
 * @param {Term|string} S
 * @param {Term|string} T 
...
function bp(S,T){
  ...
  if (S instanceof SumTerm&&(S_getLeft=S.getLeft()) instanceof PsiTerm){ //2
    if (lessThan(T,S_getLeft.sub)) return S+""; //2.1
    else return bp(S.getNotLeft(),T); //2.2
  }
  if (S instanceof PsiTerm){ //3
    var Db=S.sub;
    var c=S.inner;
    if (lessThan(T,Db)) return S+""; //3.1
    else{ //3.2
      if (lessThan(c,S)) return S+""; //3.2.1
      else return bp(c,T); //3.2.2
    }
  }
  throw Error("No rule to compute BP of "+S+" and "+T);
}
/**
 * @param {Term|string} S
...
function fund(S,T){
  ...
  if (S instanceof PsiTerm){ //3
    ...
    if (equal(Term_dom_c,"0")) return T+""; //3.1
      ...
      else return "ψ^"+Da+"_"+Db+"("+fund(c,"0")+")"; //3.2.2
    }else{ //3.3
      if (lessThan(Term_dom_c,S)) return "ψ^"+Da+"_"+Db+"("+fund(c,T)+")"; //3.3.1
      else{ //3.3.2
        var cp_c=cp(c);
        var Term_cp_c=new Term(cp_c);
        if (!(Term_cp_c instanceof PsiTerm)) throw Error("Unexpected error");
        var Dd=Term_cp_c.sup;
        var De=Term_cp_c.sub;
        var e=toNat(De);
        var Term_fund_S_fund_T_0=null;
        if (equal(T,"0")){ //3.3.2.1
          if (rank(c)==e) return "ψ^"+Da+"_"+Db+"("+fund(c,"ψ^"+Dd+"_"+fund(De,"0")+"(0)")+")"; //3.3.2.1.1
          else return "ψ^"+Da+"_"+Db+"("+fund(c,"0")+")"; //3.3.2.1.2
        }else if (isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,"0")))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sup,Da)&&equal(Term_fund_S_fund_T_0.sub,Db)) return "ψ^"+Da+"_"+Db+"("+fund(c,"ψ^"+Dd+"_"+fund(De,"0")+"("+bp(Term_fund_S_fund_T_0.inner,fund(De,"0"))+")")+")"; //3.3.2.2
        else return "ψ^"+Da+"_"+Db+"("+fund(c,T)+")"; //3.3.2.3
      }
    }
  }
...
function findOTPath(x,limit){
  ...
  if (!inRT(x)) return {isStandard:false,path:[],funds:[]};
  ...
  if (equal(x,"0")){
    ...
    var xt=new Term(x);
    var m=0;
    if (xt instanceof PsiTerm) m=toNat(xt.sup);
    var n=0;
    var t;
    while(n<=limit&&!lessThanOrEqual(x,limitOrd(m,n))){
      n++;
    };
 ...
 * @param {number} m 
 * @param {number} n 
 * @returns {string} ψ^m_0(ψ^m_n(0))
 */
function limitOrd(m,n){
  return "ψ^{"+normalizeAbbreviations(m)+"}_0(ψ^{"+normalizeAbbreviations(m)+"}_{"+normalizeAbbreviations(n)+"}(0))";
}
/**
 * @param {string} S 
 * @param {number} n 
