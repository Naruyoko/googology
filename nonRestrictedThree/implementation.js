  ...
  while (scanner.hasNext()&&state!=EXIT){
    ...
    if (nums.indexOf(next)!=-1){
      ...
      if (nextnext=="_"){
        ...
        var innerterm=Term.build(scanner,THREETERMINNER);
        var nextnext=scanner.next();
        if (nextnext!=")") throw Error("Expected closing ) at position "+(scanner.pos-1)+", instead got "+nextnext+" in expression "+scanner.s);
        appendToRSum(ThreeTerm.buildNoClone(subscriptterm,innerterm));
      }else if (nextnext=="("){
        var innerterm=Term.build(scanner,THREETERMINNER);
        var nextnext=scanner.next();
        if (nextnext!=")") throw Error("Expected closing ) at position "+(scanner.pos-1)+", instead got "+nextnext+" in expression "+scanner.s);
        appendToRSum(ThreeTerm.buildNoClone(ZeroTerm.build(),innerterm));
    ...
    if (state==CLOSEDTERM){
      ...
      if (context==BRACES&&peek=="}"){
        ...
        state=EXIT;
      }else if (context==THREETERMINNER&&peek==")"){
        state=EXIT;
      }
    }
  }
...
ThreeTerm.build=function (sub,inner){
  var r=new ThreeTerm();
  r.sub=new Term(sub);
 ...
 * @returns {ThreeTerm}
 */
ThreeTerm.buildNoClone=function (sub,inner){
  var r=new ThreeTerm();
  r.sub=sub;
  r.inner=inner;
  return r;
...
ThreeTerm.prototype.clone=function (){
  return ThreeTerm.build(this.sub,this.inner);
}
/** @param {boolean} abbreviate */
ThreeTerm.prototype.toString=function (abbreviate){
  ...
  else if (abbreviate&&this.sub.equal(Term.ZERO)) return "三("+this.inner.toString(abbreviate)+")";
  else return "三_"+this.sub.toStringWithImplicitBrace(abbreviate)+"("+this.inner.toString(abbreviate)+")";
}
ThreeTerm.prototype.equal=function (other){
  if (!(other instanceof Term)) other=new Term(other);
  return other instanceof ThreeTerm&&this.sub.equal(other.sub)&&this.inner.equal(other.inner);
}
...
function inT(t){
  ...
  if (t instanceof ThreeTerm) return inT(t.sub)&&inT(t.inner);
...
function dom(S){
  ...
  if (S instanceof ThreeTerm){ //3
    ...
    if (equal(Term_dom_S_inner,"0")){ //3.1
      var dom_S_sub=dom(S.sub);
      var Term_dom_S_sub=new Term(dom_S_sub);
      if (equal(Term_dom_S_sub,"0")||equal(Term_dom_S_sub,"1")) return S+""; //3.1.1
      else return dom_S_sub; //3.1.2
    }else if (equal(Term_dom_S_inner,"1")) return normalizeAbbreviations("ω"); //3.2
    else{ //3.3
      if (lessThan(Term_dom_S_inner,S)) return dom_S_inner; //3.3.1
      else{ //3.3.2
        if (!(Term_dom_S_inner instanceof ThreeTerm)) throw Error("Unexpected error");
        var c=Term_dom_S_inner.sub;
        var d=Term_dom_S_inner.inner;
        var dom_d=dom(d);
        var Term_dom_d=new Term(dom_d);
        if (equal(Term_dom_d,"0")) return S+""; //3.3.2.1
        else return normalizeAbbreviations("ω"); //3.3.2.2
      }
    }
  }
...
function bp(S,T){
  ...
  if (S instanceof ZeroTerm) return "0"; //1
  var S_getLeft;
  if (S instanceof SumTerm&&(S_getLeft=S.getLeft()) instanceof ThreeTerm){ //2
    var a=S_getLeft.sub;
    if (lessThan(T,a)) return S+""; //2.1
    else return bp(S.getNotLeft(),T); //2.2
  }
  if (S instanceof ThreeTerm){ //3
    if (lessThan(T,S.sub)) return S+""; //3.1
    else{ //3.2
      if (lessThan(S.inner,S)) return S+""; //3.2.1
      else return bp(S.inner,T); //3.2.2
    }
  }
...
function fund(S,T){
  ...
  if (S instanceof ThreeTerm){ //3
    ...
    if (equal(Term_dom_S_inner,"0")){ //3.1
      var dom_S_sub=dom(S.sub);
      var Term_dom_S_sub=new Term(dom_S_sub);
      if (equal(Term_dom_S_sub,"0")||equal(Term_dom_S_sub,"1")) return T+""; //3.1.1
      else return "三_"+fund(S.sub,T)+"("+S.inner+")"; //3.1.2
    }else if (equal(Term_dom_S_inner,"1")){ //3.2
      var Term_fund_T_0=null;
      if (equal(T,(Term_fund_T_0=new Term(fund(T,"0")))+"+1")) return fund(S,Term_fund_T_0)+"+"+fund(S,"1"); //3.2.1
      else return "三_"+S.sub+"("+fund(S.inner,"0")+")"; //3.2.2
    }else{ //3.3
      if (lessThan(Term_dom_S_inner,S)) return "三_"+S.sub+"("+fund(S.inner,T)+")"; //3.3.1
      else{ //3.3.2
        if (!(Term_dom_S_inner instanceof ThreeTerm)) throw Error("Unexpected error");
        ...
        if (equal(Term_dom_d,"0")){ //3.3.2.1
          if (equal(T,"0")) return "三_"+S.sub+"("+fund(S.inner,"三_"+fund(c,"0")+"(0)")+")"; //3.3.2.1.1
          else if (isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,"0")))) instanceof ThreeTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "三_"+S.sub+"("+fund(S.inner,"三_"+fund(c,"0")+"("+Term_fund_S_fund_T_0.inner+")")+")"; //3.3.2.1.2
          else return "三_"+S.sub+"("+fund(S.inner,T)+")"; //3.3.2.1.3
        }else if (Term_dom_d instanceof ThreeTerm){ //3.3.2.2
          var fund_e_0=null;
          var e=Term_dom_d.sub;
          if (equal(T,"0")) return "三_"+S.sub+"("+fund(S.inner,"0")+")"; //3.3.2.2.1
          else if (isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,"0")))) instanceof ThreeTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "三_"+S.sub+"("+fund(S.inner,"三_"+(fund_e_0=fund(e,"0"))+"("+bp(Term_fund_S_fund_T_0.inner,fund_e_0)+")")+")"; //3.3.2.2.2
          else return "三_"+S.sub+"("+fund(S.inner,T)+")"; //3.3.2.2.3
        }
      }
    }
  }
...
/**
 * @param {number} n 
 * @returns {string} 三_0(三_0(Λ(n)))
 */
function limitOrd(n){
  return "三_0(三_0("+"三_".repeat(n+1)+"0"+"(0)".repeat(n+1)+"))";
}
/**
 * @param {string} S 
 * @param {number} n 
