...
function dom(S){
  ...
  if (S instanceof PsiTerm){ //3
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)||equal(Term_dom_S_inner,Term.ONE)) return S+""; //3.1
    else if (equal(Term_dom_S_inner,Term.ONE_PLUS_SMALLOMEGA)) return Term.SMALLOMEGA+""; //3.2
    else{ //3.3
      if (!(Term_dom_S_inner instanceof PsiTerm)) throw Error("Assertion failed");
      if (equal(dom(Term_dom_S_inner.inner),Term.ONE)) return Term.SMALLOMEGA+""; //3.3.1
      else{ //3.3.2
        if (toNat(S.sub)>=toNat(Term_dom_S_inner.sub)) return dom_S_inner; //3.3.2.1
  ...
  if (S instanceof PsiTerm){ //3
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)) return T+""; //3.1
      ...
      if (equal(dom(Term_dom_S_inner.inner),Term.ONE)) return "ψ("+S.sub+","+fund(S.inner,T)+")"; //3.4.1
        ...
        if (a>=c) return "ψ("+S.sub+","+fund(S.inner,T)+")"; //3.4.2.1
        else{ //3.4.2.2
          if (c-a==1){ //3.4.2.2.1
            var Term_fund_S_fund_T_0=null;
            if (equal(dom(T),Term.ONE)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"ψ("+S.sub+","+Term_fund_S_fund_T_0.inner+")")+")"; //3.4.2.2.1.1
            else return "ψ("+S.sub+","+fund(S.inner,Term.ZERO)+")"; //3.4.2.2.1.2
          }else{ //3.4.2.2.2
            var del=c-a-1;
            var Term_fund_S_fund_T_0=null;
            if (equal(dom(T),Term.ONE)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"ψ("+normalizeAbbreviations(a+del)+","+ascend(Term_fund_S_fund_T_0.inner,del,a)+")")+")"; //3.4.2.2.2.1
            else return "ψ("+S.sub+","+fund(S.inner,Term.ZERO)+")"; //3.4.2.2.2.2
          }
        }
      }
    }
