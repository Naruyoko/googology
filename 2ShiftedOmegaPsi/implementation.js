...
function inT(t){
  ...
  if (t instanceof SumTerm) return t.terms.every(inPT); //4
  if (t instanceof PsiTerm) return inTOmega(t.sub)&&inT(t.inner); //5
  return false;
}
function inPT(t){
  try{
...
function inTOmega(t){
  ...
  if (t instanceof ZeroTerm) return true; //1
  if (t instanceof OmegaTerm) return true; //2
  if (t instanceof SumTerm) return t.terms.every(inPTOmega); //3
  return false;
}
function inPTOmega(t){
  try{
    if (!(t instanceof Term)) t=new Term(t);
  }catch(e){
    return false;
...
function dom(S){
  ...
  if (S instanceof OmegaTerm) return Term.LARGEOMEGA+""; //2
  ...
  if (S instanceof PsiTerm){ //4
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)) return S+""; //4.1
      ...
      if (lessThan(Term_dom_S_inner,S)) return dom_S_inner; //4.3.1
        ...
        if (Term_dom_S_inner instanceof OmegaTerm){ //4.3.2.1
          if (lessThan(S.sub,S.inner)) return Term.SMALLOMEGA+""; //4.3.2.1.1
          else return S+""; //4.3.2.1.2
        }
        if (Term_dom_S_inner instanceof PsiTerm){ //4.3.2.2
          if (lessThan(S.sub,Term_dom_S_inner.inner)) return Term.SMALLOMEGA+""; //4.3.2.2.1
          else return dom_S_inner; //4.3.2.2.2
        }
      }
    }
  }
  ...
  if (S instanceof PsiTerm){ //4
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)) return T+""; //4.1
      ...
      if (lessThan(Term_dom_S_inner,S)) return "ψ("+S.sub+","+fund(S.inner,T)+")"; //4.3.1
        ...
        if (Term_dom_S_inner instanceof OmegaTerm){ //4.3.2.1
          if (lessThan(S.sub,S.inner)){ //4.3.2.1.1
            var Term_fund_S_fund_T_0=null;
            if (equal(dom(T),Term.ONE)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"ψ("+fund(S.inner,Term.ZERO)+","+Term_fund_S_fund_T_0.inner+")")+")"; //4.3.2.1.1.1
            else return "ψ("+S.sub+","+fund(S.inner,Term.ZERO)+")"; //4.3.2.1.1.2
          }else return T+""; //4.3.2.1.2
        }
        if (Term_dom_S_inner instanceof PsiTerm){ //4.3.2.2
          if (lessThan(S.sub,Term_dom_S_inner.inner)){ //4.3.2.2.1
            if (Term_dom_S_inner.inner instanceof OmegaTerm){ //4.3.2.2.1.1
              var Term_fund_S_fund_T_0=null;
              if (equal(dom(T),Term.ONE)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"ψ(0,"+Term_fund_S_fund_T_0.inner+")")+")"; //4.3.2.2.1.1.1
              else return "ψ("+S.sub+","+fund(S.inner,"ψ(0,0)")+")"; //4.3.2.2.1.1.2
            }else{ //4.3.2.2.1.2
              var Term_fund_S_fund_T_0=null;
              if (equal(dom(T),Term.ONE)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"ψ("+Term_dom_S_inner.sub+","+fund(Term_dom_S_inner.inner,"ψ("+fund(Term_dom_S_inner.inner,Term.ZERO)+","+Term_fund_S_fund_T_0.inner+")")+")")+")"; //4.3.2.2.1.2.1
              else return "ψ("+S.sub+","+fund(S.inner,"ψ("+Term_dom_S_inner.sub+","+fund(Term_dom_S_inner.inner,Term.ZERO)+")")+")"; //4.3.2.2.1.2.2
            }
          }else return "ψ("+S.sub+","+fund(S.inner,T)+")"; //4.3.2.2.2
        }
      }
    }
  }
 ...
 */
function limitOrd(n){
  return "ψ(0,"+Term.LARGEOMEGA+(n==0?"":("+"+Term.LARGEOMEGA).repeat(n))+")";
}
...
var testTerms=[
  ...
  ["ψ_0(Ω+ψ_Ω(Ω+1))",-1],
  ["ψ_0(Ω+ψ_Ω(Ω+ψ_0(Ω)))",-1],
  ["ψ_0(Ω+ψ_Ω(Ω+ψ_0(Ω+ψ_Ω(Ω))))",3],
  ["ψ_0(Ω+ψ_Ω(Ω+ψ_Ω(Ω)))",3],
  ["ψ_0(Ω+Ω)",3],
  ["ψ_0(Ω+Ω+ψ_Ω+Ω(Ω))",3],
  ["ψ_0(Ω+Ω+ψ_Ω+Ω(Ω+ψ_Ω(Ω)))",3],
  ["ψ_0(Ω+Ω+ψ_Ω+Ω(Ω+ψ_Ω(Ω+Ω)))",3],
  ["ψ_0(Ω+Ω+ψ_Ω+Ω(Ω+Ω))",3],
  ["ψ_0(Ω+Ω+Ω)",3],
];
    ...
    if (oldinput!=input){
      ...
      try{
        if (cmd=="normalize"||cmd=="norm"){
          ...
          result=inPT(args[0]);
        }else if (cmd=="inTOmega"){
          result=inTOmega(args[0]);
        }else if (cmd=="inPTOmega"){
          result=inPTOmega(args[0]);
        }else if (cmd=="dom"){
          result=dom(args[0]);
