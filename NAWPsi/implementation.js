  ...
  while (scanner.hasNext()&&state!=EXIT){
    ...
    if (symbolType==NUMBER){
      ...
      if (nextnext=="_"){
        ...
        appendToRSum(PsiTerm.build(superscriptterm,subscriptterm,innerterm));
      }else if (nextnext=="^"){
        var superscriptterm=Term.build(scanner,PSITERMSUPERSCRIPT);
        var nextnext=scanner.next();
        if (nextnext!="(") throw Error("Expected ( at position "+(scanner.pos-1)+", instead got "+nextnext+" in expression "+scanner.s);
        ...
        var nextnext=scanner.next();
        if (nextnext!=")") throw Error("Expected closing ) at position "+(scanner.pos-1)+", instead got "+nextnext+" in expression "+scanner.s);
        appendToRSum(PsiTerm.build(superscriptterm,ZeroTerm.build(),innerterm));
      }else if (nextnext=="("){
        var subterms=[];
...
PsiTerm.prototype.toString=function (abbreviate){
  if (abbreviate){
    ...
    }else if ((abbreviate===true||abbreviate["3ψ"])) return "ψ_"+this.sub.toStringWithImplicitBrace(abbreviate)+"^"+this.sup.toString(abbreviate)+"("+this.inner.toString(abbreviate)+")";
  }
  return "ψ("+this.sup.toString(abbreviate)+","+this.sub.toString(abbreviate)+","+this.inner.toString(abbreviate)+")";
}
...
function dom(S){
  ...
  if (S instanceof PsiTerm){ //3
    ...
    else if (equal(Term_dom_c,Term.ONE)) return Term.SMALLOMEGA+""; //3.2
    else{ //3.3
      if (!(Term_dom_c instanceof PsiTerm)) throw Error("Unexpected error");
      var d=toNat(Term_dom_c.sup);
      var e=toNat(Term_dom_c.sub);
      if (e==0){ //3.3.1
        if (a<d) return Term.SMALLOMEGA+""; //3.3.1.1
        else return dom_c; //3.3.1.2
      }else{ //3.3.2
        if (b<e){ //3.3.2.1
          if (a<=d) return Term.SMALLOMEGA+""; //3.3.2.1.1
          else return dom_c; //3.3.2.1.2
        }else if (b==e){ //3.3.2.2
          if (a<d) return S+""; //3.3.2.2.1
...
function fund(S,T){
  ...
  if (S instanceof PsiTerm){ //3
    ...
    if (equal(Term_dom_c,Term.ZERO)) return T+""; //3.1
      ...
      else return "ψ("+S.sup+","+S.sub+","+fund(S.inner,Term.ZERO)+")"; //3.2.2
    }else{ //3.3
      if (!(Term_dom_c instanceof PsiTerm)) throw Error("Unexpected error");
      var d=toNat(Term_dom_c.sup);
      var e=toNat(Term_dom_c.sub);
      if (e==0){ //3.3.1
        if (a<d){ //3.3.1.1
          var Term_fund_S_fund_T_0;
          if (isNat(T)&&!equal(T,Term.ZERO)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm) return "ψ("+a+","+b+","+fund(c,"ψ("+a+","+b+","+Term_fund_S_fund_T_0.inner+")")+")"; //3.3.1.1.1
          else return "ψ("+a+","+b+","+fund(c,"ψ("+a+","+b+",0)")+")"; //3.3.1.1.2
        }else return "ψ("+a+","+b+","+fund(c,T)+")"; //3.3.1.2
      }else{ //3.3.2
        if (b<e){ //3.3.2.1
          if (a<=d){ //3.3.2.1.1
            var cp_c=cp(c);
            var Term_cp_c=new Term(cp_c);
            if (!(Term_cp_c instanceof PsiTerm)||!equal(Term_cp_c.inner,Term.ZERO)) throw Error("Unexpected error");
            var $g=Term_cp_c.sup;
            var h=toNat(Term_cp_c.sub);
            var Term_fund_S_fund_T_0;
            if (isNat(T)&&!equal(T,Term.ZERO)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm) return "ψ("+a+","+b+","+fund(c,"ψ("+$g+","+normalizeAbbreviations(h-1)+","+Term_fund_S_fund_T_0.inner+")")+")"; //3.3.2.1.1.1
            else return "ψ("+a+","+b+","+fund(c,"ψ("+$g+","+normalizeAbbreviations(h-1)+",0)")+")"; //3.3.2.1.1.2
          }else return "ψ("+a+","+b+","+fund(c,T)+")"; //3.3.2.1.2
        }else return "ψ("+a+","+b+","+fund(c,T)+")"; //3.3.2.2
      }
    }
  }
...
function limitOrd(n){
  var r="";
  for (var i=0;i<=n;i++){
    r+="ψ("+normalizeAbbreviations(i)+","+normalizeAbbreviations(i)+",";
  }
  r+="0"+")".repeat(n+1);
  return r;
}
