...
function inT(t){
  ...
  if (t instanceof PsiTerm) return inT(t.inner);
...
function dom(S){
  ...
  if (S instanceof PsiTerm){ //3
    ...
    }else if (equal(Term_dom_S_inner,"1")||equal(Term_dom_S_inner,"ω")) return normalizeAbbreviations("ω"); //3.2
    else{ //3.3
...
function fund(S,T){
  ...
  if (S instanceof SumTerm){ //2
    ...
    if (S.terms[m-1] instanceof PsiTerm&&equal(dom(S.terms[m-1].inner),"1")){ //2.1
      if (isNat(T)){ //2.1.1
        var i=toNat(T);
        var G,g;
        for (var k=m-1;k>=0;k--){ //2.1.1.1
          if (k>0&&S.terms[k-1] instanceof PsiTerm&&equal(S.terms[k-1].sub,S.terms[m-1].sub)&&lessThan(S.terms[k-1].inner,S.terms[m-1].inner)){ //2.1.1.1.1
  ...
  if (S instanceof PsiTerm){ //3
    ...
    if (equal(Term_dom_S_inner,"0")){ //3.1
      ...
      var fund_T_0=null;
      if (equal(T,"1")) return "ψ_"+S.sub+"("+fund(S.inner,"0")+")"; //3.2.1
      else if (equal(T,(fund_T_0=fund(T,"0"))+"+1")) return fund(S,fund_T_0)+"+"+fund(S,"1"); //3.2.2
      else return "0"; //3.2.3
    }else if (equal(Term_dom_S_inner,"ω")) return "ψ_"+S.sub+"("+fund(S.inner,T)+")"; //3.3
    else{ //3.4
      if (lessThan(Term_dom_S_inner,S)) return "ψ_"+S.sub+"("+fund(S.inner,T)+")"; //3.4.1
      else{ //3.4.2
        if (!(Term_dom_S_inner instanceof PsiTerm)) throw Error("Invalid argument: "+Term_dom_S_inner);
        var c=Term_dom_S_inner.sub;
        var Term_fund_S_fund_T_0=null;
        if (isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,"0")))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ_"+S.sub+"("+fund(S.inner,"ψ_"+fund(c,"0")+"("+Term_fund_S_fund_T_0.inner+")")+")"; //3.4.2.1
        else return "ψ_"+S.sub+"("+fund(S.inner,"ψ_"+fund(c,"0")+"(0)")+")"; //3.4.2.2
      }
    }
  }
