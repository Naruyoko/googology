...
Term.build=function (s){
  ...
  s=s.replaceAll("p","ψ").replaceAll("w","ω").replaceAll("W","Ω").replaceAll("II","ℐ");
  if (!/^[ψ_ωΩI0-9\(\)\[\]{}\+ℐ^]+$/.test(s)) throw Error("Invalid expression: "+strin);
  var nums="0123456789";
  ...
  function alphaToNum(s){
    if (s[0]!="<"||s[s.length-1]!=">") throw Error("Invalid key: "+s);
    s=s.slice(1,-1);
    for (var i=0;i<10;i++){
  ...
  s=s.replace(/Ω_[0-9]+/g,function (s){
    var n=+s.slice(2);
    if (n==1){
      return "ψ^1_I(0)";
    }else if (n>=2){
  ...
  s=s.replace(/I_[0-9]+/g,function (s){
    var n=+s.slice(2);
    if (n==1){
      return "ψ^2_ℐ[2](0)";
    }else if (n>=2){
  ...
  while (true){
    ...
    if (s.indexOf("ω")!=-1){ //ω
      manipulated=true;
      s=s.replaceAll("ω",newSubterm(Term("ψ(1)")));
    }
    if (s.indexOf("Ω")!=-1){ //Ω
      manipulated=true;
      s=s.replaceAll("Ω",newSubterm(Term("ℐ[0]")));
    }
    if (s.indexOf("I")!=-1){ //I
      manipulated=true;
      s=s.replaceAll("I",newSubterm(Term("ℐ[1]")));
    }
    if (s.indexOf("0")!=-1){ //0
      manipulated=true;
    ...
    if (s.indexOf("1")!=-1){ //1
      manipulated=true;
      s=s.replaceAll("1",newSubterm(Term("ψ(0)")));
    }
    while (/{<[a-j]+>}/.test(s)){ //{#}->#
      manipulated=true;
      s=s.replace(/{<[a-j]+>}/g,function (s){return s.slice(1,-1);});
    }
    if (/ψ\(<[a-j]+>\)/.test(s)){ //ψ(#)->ψ^0_Ω(#)
      manipulated=true;
      s=s.replace(/ψ\(<[a-j]+>\)/g,function (s){return "ψ^0_Ω"+s.slice(1);});
    }
    while (/ψ\^<[a-j]+>_<[a-j]+>\(<[a-j]+>\)/.test(s)){ //ψ^#_#(#)->#
      manipulated=true;
      s=s.replace(/ψ\^<[a-j]+>_<[a-j]+>\(<[a-j]+>\)/g,function (s){return newSubterm(PsiTerm.build(getSubterm(s.slice(2,s.indexOf("_"))),getSubterm(s.slice(s.indexOf("_")+1,s.indexOf("("))),getSubterm(s.slice(s.indexOf("(")+1,-1))));});
    }
    while (/ℐ\[<[a-j]+>\]/.test(s)){ //ℐ[#]->#
      manipulated=true;
      s=s.replace(/ℐ\[<[a-j]+>\]/g,function (s){return newSubterm(IITerm.build(getSubterm(s.slice(2,-1))));});
    }
    if (/(?<!\+)<[a-j]+>(\+<[a-j]+>)+(?!\+)/.test(s)){ //#+#+...+#->#
      manipulated=true;
