  ...
  if (S instanceof PsiTerm){ //3
    ...
    if (equal(Term_dom_b,"0")) return S+""; //3.1
      ...
      if (lessThan(Term_dom_b,S)) return dom_b; //3.3.1
        ...
        var rank_b=rank(b);
        if (rank_b==0) return normalizeAbbreviations("ω"); //3.3.2.1
...
function rank(S){
  ...
  if (S instanceof PsiTerm){ //3
    ...
    var dom_b=dom(b);
    var Term_dom_b=new Term(dom_b);
    if (equal(Term_dom_b,"0")) return a; //3.1
    else if (equal(Term_dom_b,"1")) return 0; //3.2
    else{ //3.3
      if (lessThan(Term_dom_b,S)) return rank(b); //3.3.1
      else{ //3.3.2
        var rank_b=rank(b);
        if (rank_b==0) return 0; //3.3.2.1
        else return rank_b-1; //3.3.2.2
      }
    }
  }
...
function bp(S,T){
  ...
  if (S instanceof PsiTerm){ //3
    var Da=S.sub;
    var b=S.inner;
    if (lessThan(T,Da)) return S+""; //3.1
    else{ //3.2
      if (lessThan(b,S)) return S+""; //3.2.1
      else return bp(b,T); //3.2.2
    }
  }
  ...
  if (S instanceof PsiTerm){ //3
    ...
    if (equal(Term_dom_b,"0")) return T+""; //3.1
      ...
      if (lessThan(Term_dom_b,S)) return "ψ_"+Da+"("+fund(b,T)+")"; //3.3.1
        ...
        var Dc=Term_cp_b.sub;
        var c=toNat(Dc);
        var Term_fund_S_fund_T_0=null;
        if (equal(T,"0")){ //3.3.2.1
          if (rank(b)==c) return "ψ_"+Da+"("+fund(b,"ψ_"+fund(Dc,"0")+"(0)")+")"; //3.3.2.1.1
          else return "ψ_"+Da+"("+fund(b,"0")+")"; //3.3.2.1.2
        }else if (isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,"0")))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,Da)) return "ψ_"+Da+"("+fund(b,"ψ_"+fund(Dc,"0")+"("+bp(Term_fund_S_fund_T_0.inner,fund(Dc,"0"))+")")+")"; //3.3.2.2
        else return "ψ_"+Da+"("+fund(b,T)+")"; //3.3.2.3
      }
    }
  }
