...
OmegaTerm.prototype.equal=function (other){
  if (!(other instanceof Term)) other=new Term(other);
  return other instanceof OmegaTerm;
}
...
function inT(t){
  ...
  if (t instanceof OmegaTerm) return true; //2
...
function inRT(t){
  ...
  if (t instanceof ZeroTerm) return true; //1
  if (t instanceof OmegaTerm) return true; //2
  if (t instanceof SumTerm) return t.terms.every(inRPT); //3
  if (t instanceof PsiTerm) return inRT(t.sub)&&inRT(t.inner); //4
  return false;
}
function inRPT(t){
  try{
  ...
  if (t instanceof OmegaTerm) return true; //2
  if (t instanceof PsiTerm) return isNat(t.sub)&&inRT(t.inner); //4
  return false;
}
/**
 * @param {Term} t
...
function lessThan(S,T){
  ...
  if (S instanceof OmegaTerm){ //3
    if (T instanceof OmegaTerm) return false; //3.1
    if (T instanceof SumTerm) return lessThanOrEqual(S,T.getLeft()); //3.2
    if (T instanceof PsiTerm) return false; //3.3
  }
  if (S instanceof SumTerm){ //4
    if (T instanceof OmegaTerm) return lessThan(S.getLeft(),T); //4.1
    if (T instanceof SumTerm) //4.2
      return lessThan(S.getLeft(),T.getLeft()) //4.2.1
        ||equal(S.getLeft(),T.getLeft())&&lessThan(S.getNotLeft(),T.getNotLeft()); //4.2.2
    if (T instanceof PsiTerm) return lessThan(S.getLeft(),T); //4.3
  }
  if (S instanceof PsiTerm){ //5
    if (T instanceof OmegaTerm) return true; //5.1
    if (T instanceof SumTerm) return lessThanOrEqual(S,T.getLeft()); //5.2
    if (T instanceof PsiTerm){ //5.3
      var a=toNat(S.sub);
      var c=toNat(T.sub);
      return a<c //5.3.1
        ||a==c&&lessThan(S.inner,T.inner); //5.3.2
    }
  }
...
function dom(S){
  ...
  if (S instanceof OmegaTerm) return "Ω"; //2
  if (S instanceof SumTerm) return dom(S.getRight()); //3
  if (S instanceof PsiTerm){ //4
    ...
    else if (equal(Term_dom_S_inner,Term.ONE)||equal(Term_dom_S_inner,Term.LARGEOMEGA)) return Term.SMALLOMEGA+""; //4.2
    else{ //4.3
...
function level(S){
  ...
  if (!inRT(S)) throw Error("Invalid argument: "+S);
  if (S instanceof ZeroTerm) return 0; //1
  if (S instanceof OmegaTerm) return 1; //2
  if (S instanceof SumTerm) return S.terms.reduce(function (a,c){return a+level(c);},0); //3
  if (S instanceof PsiTerm) return toNat(S.sub); //4
  throw Error("No rule to compute level("+S+")");
}
/**
 * @param {Term|string} S 
...
function fund(S,T){
  ...
  if (!inRT(S)||!inRT(T)) throw Error("Invalid argument: "+S+","+T);
  if (S instanceof ZeroTerm) return "0"; //1
  if (S instanceof OmegaTerm) return T+""; //2
  if (S instanceof SumTerm){ //3
    var fund_S_getRight_T=fund(S.getRight(),T);
    var Term_fund_S_getRight_T=new Term(fund_S_getRight_T);
    if (equal(Term_fund_S_getRight_T,Term.ZERO)) return S.getNotRight()+""; //3.1
    else return S.getNotRight()+"+"+fund_S_getRight_T; //3.2
  }
  if (S instanceof PsiTerm){ //4
    var dom_S_inner=dom(S.inner);
    var Term_dom_S_inner=new Term(dom_S_inner);
    if (equal(Term_dom_S_inner,Term.ZERO)){ //4.1
      ...
      else if (equal(Term_dom_S_sub,Term.LARGEOMEGA)){ //4.1.2
        var Term_fund_S_fund_T_0=null;
        if (isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.inner,S.inner)) return "ψ("+fund(S.sub,"ψ("+Term_fund_S_fund_T_0.sub+",0)")+","+S.inner+")"; //4.1.2.1
        else return "ψ("+fund(S.sub,"ψ(0,0)")+","+S.inner+")"; //4.1.2.2
      }else return "ψ("+fund(S.sub,T)+","+S.inner+")"; //4.1.3
    ...
    }else if (equal(Term_dom_S_inner,Term.LARGEOMEGA)){ //4.3
      var level_S=level(S);
      var level_S_inner=level(S.inner);
      if (level_S<level_S_inner){ //4.3.1
        var Term_fund_S_fund_T_0=null;
        if (notEqual(T,Term.ZERO)&&isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"ψ("+normalizeAbbreviations(level_S_inner-1)+","+Term_fund_S_fund_T_0.inner+")")+")"; //4.3.1.1
        else return "ψ("+S.sub+","+fund(S.inner,"ψ("+normalizeAbbreviations(level_S_inner-1)+",0)")+")"; //4.3.1.2
      }else{ //4.3.2
        var Term_fund_S_fund_T_0=null;
        if (notEqual(T,Term.ZERO)&&isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"ψ("+S.sub+","+Term_fund_S_fund_T_0.inner+")")+")"; //4.3.2.1
        else return "ψ("+S.sub+","+fund(S.inner,"ψ("+S.sub+",0)")+")"; //4.3.2.2
      }
    }else{ //4.4
      if (lessThan(Term_dom_S_inner,S)) return "ψ("+S.sub+","+fund(S.inner,T)+")"; //4.4.1
      else{ //4.4.2
        if (!(Term_dom_S_inner instanceof PsiTerm)) throw Error("Unexpected error");
        var $c=Term_dom_S_inner.sub;
        var Term_fund_S_fund_T_0=null;
        if (notEqual(T,Term.ZERO)&&isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"ψ("+fund($c,Term.ZERO)+","+Term_fund_S_fund_T_0.inner+")")+")"; //4.4.2.1
        else return "ψ("+S.sub+","+fund(S.inner,"ψ("+fund($c,Term.ZERO)+",0)")+")"; //4.4.2.2
      }
    }
  }
...
var testTermsPre=[
  ...
  ["ψ_0(Ω+Ω)",3],
  ["ψ_0(Ω+Ω+ψ_0(Ω))",3],
  ["ψ_0(Ω+Ω+ψ_0(Ω+ψ_1(Ω)))",3],
  ["ψ_0(Ω+Ω+ψ_0(Ω+Ω))",3],
  ["ψ_0(Ω+Ω+ψ_1(0))",3],
  ["ψ_0(Ω+Ω+ψ_1(Ω+Ω))",3],
  ["ψ_0(Ω+Ω+ψ_2(0))",3],
  ["ψ_0(Ω+Ω+ψ_2(ψ_0(Ω+ψ_1(Ω))))",3],
  ["ψ_0(Ω+Ω+ψ_2(ψ_0(Ω+Ω)))",3],
  ["ψ_0(Ω+Ω+ψ_2(ψ_1(0)))",3],
  ["ψ_0(Ω+Ω+ψ_2(ψ_1(Ω)))",3],
  ["ψ_0(Ω+Ω+ψ_2(ψ_1(Ω+Ω)))",3],
  ["ψ_0(Ω+Ω+ψ_2(ψ_2(0)))",3],
  ["ψ_0(Ω+Ω+ψ_2(Ω))",3],
  ["ψ_0(Ω+Ω+ψ_2(Ω)+ψ_1(0))",3],
  ["ψ_0(Ω+Ω+ψ_2(Ω)+ψ_2(0))",3],
  ["ψ_0(Ω+Ω+ψ_2(Ω)+ψ_2(Ω))",3],
  ["ψ_0(Ω+Ω+ψ_2(Ω+1))",-1],
  ["ψ_0(Ω+Ω+ψ_2(Ω+ψ_2(0)))",3],
  ["ψ_0(Ω+Ω+ψ_2(Ω+ψ_2(Ω)))",3],
  ["ψ_0(Ω+Ω+ψ_2(Ω+Ω))",3],
