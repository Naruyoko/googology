...
Term.build=function (s,context){
  ...
  var TOP=0;
  var ITERMSUBSCRIPT=1;
  var ITERMINNER=2;
  var PSITERMSUBSCRIPT=3;
  var PSITERMINNER=4;
  var BRACES=5;
  var contextNames=["TOP","ITERMSUBSCRIPT","ITERMINNER","PSITERMSUBSCRIPT","PSITERMINNER","BRACES"];
  ...
  while (scanner.hasNext()&&state!=EXIT){
    ...
    if (symbolType==NUMBER){
      ...
      if (nextnext=="_"){
        subterms.push(Term.build(scanner,ITERMSUBSCRIPT));
        var nextnext=scanner.next();
      ...
      while (subterms.length<2){
        subterms.push(Term.build(scanner,ITERMINNER));
        var nextnext=scanner.next();
        if (nextnext==","){
      ...
      appendToRSum(ITerm.buildNoClone.apply(null,subterms));
    ...
    if (state==CLOSEDTERM){
      ...
      if (context==BRACES&&peek=="}"){
        ...
        state=EXIT;
      }else if (context==ITERMSUBSCRIPT&&peek=="("){
        state=EXIT;
      }else if (context==ITERMINNER&&(peek==","||peek==")")){
        state=EXIT;
      }
    }
  }
...
ITerm.build=function (sub,inner){
  var r=new ITerm();
  r.sub=new Term(sub);
 ...
 * @returns {ITerm}
 */
ITerm.buildNoClone=function (sub,inner){
  var r=new ITerm();
  r.sub=sub;
  r.inner=inner;
  return r;
...
ITerm.prototype.clone=function (){
  return ITerm.build(this.sub,this.inner);
}
/** @param {boolean} abbreviate */
ITerm.prototype.toString=function (abbreviate){
  if (abbreviate){
    if ((abbreviate===true||abbreviate["1I"])&&this.sub.equal(Term.ZERO)) return "I("+this.inner.toString(abbreviate)+")";
    else if ((abbreviate===true||abbreviate["2I"])) return "I_"+this.sub.toString(abbreviate)+"("+this.inner.toString(abbreviate)+")";
  }
  return "I("+this.sub.toString(abbreviate)+","+this.inner.toString(abbreviate)+")";
}
ITerm.prototype.equal=function (other){
  if (!(other instanceof Term)) other=new Term(other);
  return other instanceof ITerm&&this.sub.equal(other.sub)&&this.inner.equal(other.inner);
}
...
function inT(t){
  ...
  if (t instanceof ITerm) return inTPsi(t.sub)&&inTPsi(t.inner); //4
...
function inTPsi(t){
  ...
  if (t instanceof ZeroTerm) return true; //1
  if (t instanceof SumTerm) return t.terms.every(inPTPsi); //3
  if (t instanceof PsiTerm) return inT(t.sub)&&inTPsi(t.inner); //5
  return false;
}
function inPTPsi(t){
...
function lessThan(S,T){
  ...
  if (S instanceof PsiTerm){ //5
    ...
    if (T instanceof ITerm) return true; //5.2
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)){ //4.1
      ...
      if (equal(Term_dom_S_sub,Term.ZERO)||equal(Term_dom_S_sub,Term.ONE)) return S+""; //4.1.1
        ...
        if (lessThan(Term_dom_S_sub,S)) return dom_S_sub; //4.1.2.1
          ...
          if (Term_dom_S_sub instanceof ITerm) return S+""; //4.1.2.2.1
          else if (Term_dom_S_sub instanceof PsiTerm) return Term.SMALLOMEGA+""; //4.1.2.2.2
        }
      }
...
function fund(S,T){
  ...
  if (S instanceof ITerm){ //3
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)){ //3.1
      ...
      else return "I("+fund(S.sub,T)+","+S.inner+")"; //3.1.2
      ...
      else return "I("+S.sub+","+fund(S.inner,Term.ZERO)+")"; //3.2.2
    }else return "I("+S.sub+","+fund(S.inner,T)+")"; //3.3
  }
  if (S instanceof PsiTerm){ //4
    var dom_S_inner=dom(S.inner);
    var Term_dom_S_inner=new Term(dom_S_inner);
    if (equal(Term_dom_S_inner,Term.ZERO)){ //4.1
      ...
      if (equal(Term_dom_S_sub,Term.ZERO)||equal(Term_dom_S_sub,Term.ONE)) return T+""; //4.1.1
        ...
        if (lessThan(Term_dom_S_sub,S)) return "ψ("+fund(S.sub,T)+","+S.inner+")"; //4.1.2.1
          ...
          if (Term_dom_S_sub instanceof ITerm) return "ψ("+fund(S.sub,T)+","+S.inner+")"; //4.1.2.2.1
          else if (Term_dom_S_sub instanceof PsiTerm){ //4.1.2.2.2
            var c=Term_dom_S_sub.sub;
            var dom_c=dom(c);
            var Term_dom_c=new Term(dom_c);
            if (!(Term_dom_c instanceof ITerm&&equal(Term_dom_c.inner,Term.ZERO))) throw Error("Unexpected error");
            var d=Term_dom_c.sub;
            if (equal(d,Term.ZERO)){ //4.1.2.2.2.1
              var Term_fund_S_fund_T_0=null;
              if (notEqual(T,Term.ZERO)&&isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.inner,S.inner)) return "ψ("+fund(S.sub,bp(Term_fund_S_fund_T_0.sub))+","+S.inner+")"; //4.1.2.2.2.1.1
              else return "ψ("+fund(S.sub,"ψ(0,0)")+","+S.inner+")"; //4.1.2.2.2.1.2
            }else{ //4.1.2.2.2.2
              var Term_fund_S_fund_T_0=null;
              if (notEqual(T,Term.ZERO)&&isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.inner,S.inner)) return "ψ("+fund(S.sub,"I("+fund(d,Term.ZERO)+","+bp(Term_fund_S_fund_T_0.sub)+")")+","+S.inner+")"; //4.1.2.2.2.2.1
              else return "ψ("+fund(S.sub,"I("+fund(d,Term.ZERO)+",0)")+","+S.inner+")"; //4.1.2.2.2.2.2
            }
          }
        }
      }
      ...
      if (lessThan(Term_dom_S_inner,S)) return "ψ("+S.sub+","+fund(S.inner,T)+")"; //4.3.1
        ...
        if (!(Term_dom_S_inner instanceof PsiTerm&&equal(Term_dom_S_inner.inner,Term.ZERO))) throw Error("Unexpected error");
        var c=Term_dom_S_inner.sub;
        var dom_c=dom(c);
        var Term_dom_c=new Term(dom_c);
        if (equal(Term_dom_c,Term.ONE)){ //4.3.2.1
          var Term_fund_S_fund_T_0=null;
          if (notEqual(T,Term.ZERO)&&isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"ψ("+fund(c,Term.ZERO)+","+Term_fund_S_fund_T_0.inner+")")+")"; //4.3.2.1.1
          else return "ψ("+S.sub+","+fund(S.inner,"ψ("+fund(c,Term.ZERO)+",0)")+")"; //4.3.2.1.2
        }else if (Term_dom_c instanceof ITerm&&equal(Term_dom_c.inner,Term.ZERO)){ //4.3.2.2
          var d=Term_dom_c.sub;
          if (equal(d,Term.ZERO)){ //4.3.2.2.1
            var Term_fund_S_fund_T_0=null;
            if (notEqual(T,Term.ZERO)&&isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,Term_fund_S_fund_T_0.inner)+")"; //4.3.2.2.1.1
            else return "ψ("+S.sub+","+fund(S.inner,"ψ(0,0)")+")"; //4.3.2.2.1.2
          }else{ //4.3.2.2.2
            var Term_fund_S_fund_T_0=null;
            if (notEqual(T,Term.ZERO)&&isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"I("+fund(d,Term.ZERO)+","+Term_fund_S_fund_T_0.inner+")")+")"; //4.3.2.2.2.1
            else return "ψ("+S.sub+","+fund(S.inner,"I("+fund(d,Term.ZERO)+",0)")+")"; //4.3.2.2.2.2
          }
        }
      }
    }
 ...
 */
function limitOrd(n){
  return "ψ(0,"+"ψ(I(".repeat(n+1)+"0"+",0),0)".repeat(n+1)+")";
}
/**
 * @param {number} n 
 * @returns {number}
...
var testTermsPre=[
  ...
  ["ψ_0(ψ_ψ_ω(0)(0))",3],
  ["ψ_0(ψ_I_0(0)(0))",3],
  ["ψ_0(ψ_I_0(0)(0)+ψ_ψ_I_0(0)(0)(0))",3],
  ["ψ_0(ψ_I_0(0)(0)+ψ_ψ_I_0(0)(0)(ψ_I_0(0)(0)))",3],
  ["ψ_0(ψ_I_0(0)(0)+ψ_ψ_I_0(0)(0)+1(0))",3],
  ["ψ_0(ψ_I_0(0)(0)+ψ_I_0(0)(0))",3],
  ["ψ_0(ψ_I_0(0)(1))",-1],
  ["ψ_0(ψ_I_0(0)(ψ_I_0(0)(0)))",3],
  ["ψ_0(ψ_I_0(0)+1(0))",3],
  ["ψ_0(ψ_I_0(0)+ψ_I_0(0)(0)(0))",3],
  ["ψ_0(ψ_I_0(0)+ψ_I_0(0)(0)+ψ_I_0(0)(0)(0))",3],
  ["ψ_0(ψ_I_0(0)+ψ_I_0(0)(ψ_I_0(0)(0))(0))",3],
  ["ψ_0(ψ_I_0(0)+ψ_I_0(0)+1(0)(0))",3],
  ["ψ_0(ψ_I_0(0)+I_0(0)(0))",3],
  ["ψ_0(ψ_I_0(1)(0))",3],
  ["ψ_0(ψ_I_0(1)(0)+ψ_I_0(0)(0))",3],
  ["ψ_0(ψ_I_0(1)(0)+ψ_I_0(0)+I_0(0)(0))",3],
  ["ψ_0(ψ_I_0(1)(0)+ψ_I_0(1)(0))",-1],
  ["ψ_0(ψ_I_0(1)(ψ_I_0(0)(0)))",-1],
  ["ψ_0(ψ_I_0(1)(ψ_I_0(0)+1(0)))",-1],
  ["ψ_0(ψ_I_0(1)(ψ_I_0(0)+I_0(0)(0)))",-1],
  ["ψ_0(ψ_I_0(1)(ψ_I_0(1)(0)))",3],
  ["ψ_0(ψ_I_0(1)+1(0))",3],
  ...
  ["ψ_0(ψ_I_0(ω)(0))",-1],
  ["ψ_0(ψ_I_0(ψ_1(0))(0))",-1],
  ["ψ_0(ψ_I_0(ψ_ψ_I_0(0)(0)(0))(0))",-1],
  ["ψ_0(ψ_I_0(ψ_I_0(0)(0))(0))",3],
  ["ψ_0(ψ_I_1(0)(0))",3],
  ["ψ_0(ψ_I_1(0)(0)+ψ_I_0(0)(0))",3],
  ["ψ_0(ψ_I_1(0)(0)+ψ_I_0(0)(ψ_I_1(0)(0)))",3],
  ["ψ_0(ψ_I_1(0)(0)+ψ_I_0(ψ_I_1(0)(0))(0))",3],
  ["ψ_0(ψ_I_1(0)(0)+ψ_I_1(0)(0))",3],
  ["ψ_0(ψ_I_1(0)(1))",-1],
  ["ψ_0(ψ_I_1(0)+1(0))",-1],
  ["ψ_0(ψ_I_1(0)+ψ_I_0(0)(0)(0))",3],
  ["ψ_0(ψ_I_1(0)+ψ_I_1(0)(0)(0))",3],
  ["ψ_0(ψ_I_1(0)+I_0(0)(0))",3],
  ["ψ_0(ψ_I_1(0)+I_0(ψ_I_0(0)(0))(0))",3],
  ["ψ_0(ψ_I_1(0)+I_0(ψ_I_1(0)(0))(0))",3],
  ["ψ_0(ψ_I_1(0)+I_1(0)(0))",3],
  ["ψ_0(ψ_I_1(1)(0))",-1],
  ["ψ_0(ψ_I_2(0)(0))",3],
  ["ψ_0(ψ_I_ω(0)(0))",3],
  ["ψ_0(ψ_I_ψ_1(0)(0)(0))",3],
  ["ψ_0(ψ_I_ψ_I_0(0)(0)(0)(0))",3]
];
/** @type {string[]}} */
var testTerms;
