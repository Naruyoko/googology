 ...
 */
function NTerm(s){
  if (s instanceof NTerm) return s.clone();
  else if (s instanceof Term&&typeof s!="string") throw Error("Invalid expression: "+s);
  if (!(this instanceof NTerm)) return new NTerm(s);
  Term.call(this,s);
  if (s&&!(this instanceof NTerm)) throw Error("Invalid expression: "+s);
}
Object.assign(NTerm,Term);
NTerm.build=function (){
  var r=new NTerm();
  return r;
}
NTerm.buildNoClone=function (){
  var r=new NTerm();
  return r;
}
NTerm.prototype=Object.create(Term.prototype);
NTerm.prototype.clone=function (){
  return NTerm.build();
}
NTerm.prototype.toString=function (abbreviate){
  return "N";
}
NTerm.prototype.equal=function (other){
  if (!(other instanceof Term)) other=new Term(other);
  return other instanceof NTerm;
}
Object.defineProperty(NTerm.prototype,"constructor",{
  value:NTerm,
  enumerable:false,
  writable:true
});
...
function inT(t){
  ...
  if (t instanceof NTerm) return true; //2
...
function lessThan(S,T){
  ...
  if (S instanceof NTerm){ //3
    if (T instanceof NTerm) return false; //3.1
  ...
  if (S instanceof SumTerm){ //4
    if (T instanceof NTerm) return lessThan(S.getLeft(),T); //4.1
  ...
  if (S instanceof PsiTerm){ //5
    if (T instanceof NTerm) return true; //5.1
...
function dom(S){
  ...
  if (S instanceof NTerm) return "N"; //2
  ...
  if (S instanceof PsiTerm){ //4
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)){ //4.1
      ...
      else if (equal(Term_dom_S_sub,Term.N)) return Term.SMALLOMEGA+""; //4.1.2
      else return dom_S_sub; //4.1.3
    }else if (equal(Term_dom_S_inner,Term.ONE)) return Term.SMALLOMEGA+""; //4.2
    else if (equal(Term_dom_S_inner,Term.N)){ //4.3
      if (equal(dom(S.sub),Term.N)) return S+""; //4.3.1
...
function fund(S,T){
  ...
  if (!inT(S)||!inT(T)) throw Error("Invalid argument: "+S+","+T);
  if (S instanceof ZeroTerm) return "0"; //1
  if (S instanceof NTerm) return T+""; //2
  ...
  if (S instanceof PsiTerm){ //4
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)){ //4.1
      ...
      else if (equal(Term_dom_S_sub,Term.N)){ //4.1.2
      ...
      if (equal(dom(S.sub),Term.N)) return "ψ("+S.sub+","+fund(S.inner,T)+")"; //4.3.1
      else{ //4.3.2
        var Term_fund_S_fund_T_0=null;
        if (isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"ψ("+Term_fund_S_fund_T_0.inner+",0)")+")"; //4.3.2.1
        else return "ψ("+S.sub+","+fund(S.inner,"ψ(0,0)")+")"; //4.3.2.2
      }
    }else{ //4.4
      if (lessThan(Term_dom_S_inner,S)) return "ψ("+S.sub+","+fund(S.inner,T)+")"; //4.4.1
        ...
        if (equal(dom(c),Term.N)){ //4.4.2.1
          ...
          else return "ψ("+S.sub+","+fund(S.inner,"ψ("+fund(c,Term.ZERO)+",0)")+")"; //4.4.2.2.2
        }
      }
    }
  }
...
var testTermsPre=[
  ...
  ["ψ_0(N+ψ_{N+1}(N))",3],
  ["ψ_0(N+N)",3],
  ["ψ_0(N+N+ψ_N(ψ_{N+N}(0)))",3],
  ["ψ_0(N+N+ψ_N(ψ_{N+N+1}(0)))",3],
  ["ψ_0(N+N+ψ_N(N))",3],
  ["ψ_0(N+N+ψ_N(N+ψ_{N+N}(0)))",3],
  ["ψ_0(N+N+ψ_N(N+ψ_{N+N+ψ_N(N)}(0)))",3],
  ["ψ_0(N+N+ψ_N(N+N))",3],
  ["ψ_0(N+N+ψ_{N+N}(0))",3],
  ["ψ_0(N+N+ψ_{N+N}(N))",3],
  ["ψ_0(N+N+ψ_{N+N}(N)+ψ_{N+N}(ψ_{N+N}(0)))",3],
