...
Term.build=function (s,context){
  ...
  var PSITERMINNER=2;
  var THETATERMINNER=3;
  var BRACES=4;
  var contextNames=["TOP","PSITERMSUBSCRIPT","PSITERMINNER","THETATERMINNER","BRACES"];
  ...
  while (scanner.hasNext()&&state!=EXIT){
    ...
    if (state==CLOSEDTERM){
      ...
      if (context==BRACES&&peek=="}"){
        ...
        state=EXIT;
      }else if (context==THETATERMINNER&&peek==")"){
        state=EXIT;
      }
    }
  }
...
ThetaTerm.build=function (inner){
  var r=new ThetaTerm();
  r.inner=new Term(inner);
  return r;
}
 ...
 * @returns {ThetaTerm}
 */
ThetaTerm.buildNoClone=function (inner){
  var r=new ThetaTerm();
  r.inner=inner;
  return r;
}
...
ThetaTerm.prototype.clone=function (){
  return ThetaTerm.build(this.inner);
}
/** @param {boolean} abbreviate */
ThetaTerm.prototype.toString=function (abbreviate){
  return "θ("+this.inner.toString(abbreviate)+")";
}
ThetaTerm.prototype.equal=function (other){
  if (!(other instanceof Term)) other=new Term(other);
  return other instanceof ThetaTerm&&this.inner.equal(other.inner);
}
...
function inTTheta(t){
  ...
  if (t instanceof ThetaTerm) return inT(t.inner);
...
function lessThan(S,T){
  ...
  if (S instanceof ThetaTerm){ //4
    ...
    if (T instanceof ThetaTerm) return lessThan(S.inner,T.inner); //4.2
...
function dom(S){
  ...
  if (S instanceof ThetaTerm){ //3
    ...
    if (equal(Term_dom_S_inner,"0")) return S+""; //3.1
    else if (equal(Term_dom_S_inner,"1")) return normalizeAbbreviations("ω"); //3.2
    else if (equal(Term_dom_S_inner,"θ(0)")) return "θ(θ(0))"; //3.3
    else return dom_S_inner; //3.4
  }
  if (S instanceof PsiTerm){ //4
    ...
    if (equal(Term_dom_S_inner,"0")){ //4.1
      ...
      if (equal(Term_dom_S_sub,"0")||equal(Term_dom_S_sub,"θ(0)")||equal(Term_dom_S_sub,"θ(θ(0))")) return S+""; //4.1.1
      else{ //4.1.2
...
function bp(S){
  ...
  if (S instanceof ThetaTerm) return bp(S.inner); //3
...
function fund(S,T){
  ...
  if (S instanceof ThetaTerm){ //3
    var dom_S_inner=dom(S.inner);
    var Term_dom_S_inner=new Term(dom_S_inner);
    if (equal(Term_dom_S_inner,"0")) return T+""; //3.1
      ...
      else return "θ("+fund(S.inner,"0")+")"; //3.2.2
    }else return "θ("+fund(S.inner,T)+")"; //3.3
  }
  if (S instanceof PsiTerm){ //4
    var dom_S_inner=dom(S.inner);
    var Term_dom_S_inner=new Term(dom_S_inner);
    if (equal(Term_dom_S_inner,"0")){ //4.1
      var dom_S_sub=dom(S.sub);
      var Term_dom_S_sub=new Term(dom_S_sub);
      if (equal(Term_dom_S_sub,"0")||equal(Term_dom_S_sub,"θ(0)")) return T+""; //4.1.1
      else if (equal(Term_dom_S_sub,"θ(θ(0))")) return "ψ("+fund(S.sub,T)+","+S.inner+")"; //4.1.2
      else{ //4.1.3
      ...
      if (lessThan(Term_dom_S_inner,S)) return "ψ("+S.sub+","+fund(S.inner,T)+")"; //4.3.1
        ...
        if (equal(dom(Term_dom_S_inner.sub),"θ(0)")){ //4.3.2.1
 ...
 */
function limitOrd(n){
  return "ψ(0,ψ("+"θ(".repeat(n+1)+"0"+")".repeat(n+1)+",0))";
}
/**
 * @param {string} X 
 * @param {number} n 
