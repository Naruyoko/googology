...
function dom(S){
  ...
  if (S instanceof ThetaTerm){ //3
    ...
    if (equal(Term_dom_S_inner2,"0")){ //3.1
      ...
      if (equal(Term_dom_S_inner1,"0")||equal(Term_dom_S_inner1,"1")) return S+""; //3.1.1
      else if (equal(Term_dom_S_inner1,"θ(0,0)")) return "θ(0,θ(0,0))"; //3.1.2
      else return dom_S_inner1; //3.1.3
      ...
      if (lessThan(Term_dom_S_inner2,S)) return dom_S_inner2; //3.4.1
      else return normalizeAbbreviations("ω"); //3.4.2
    }
  }
...
function fund(S,T){
  ...
  if (S instanceof ThetaTerm){ //3
    ...
    if (equal(Term_dom_S_inner2,"0")){ //3.1
      var dom_S_inner1=dom(S.inner1);
      var Term_dom_S_inner1=new Term(dom_S_inner1);
      if (equal(Term_dom_S_inner1,"0")||equal(Term_dom_S_inner1,"1")) return T+""; //3.1.1
      else return "θ("+fund(S.inner1,T)+","+S.inner2+")"; //3.1.2
      ...
      if (lessThan(Term_dom_S_inner2,S)) return "θ("+S.inner1+","+fund(S.inner2,T)+")"; //3.3.1
      else{ //3.3.2
        if (!(Term_dom_S_inner2 instanceof ThetaTerm)||!equal(Term_dom_S_inner2.inner2,"0")) throw Error("Unexpected error");
        var Term_fund_S_fund_T_0=null;
        if (isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,"0")))) instanceof ThetaTerm&&equal(Term_fund_S_fund_T_0.inner1,S.inner1)) return "θ("+S.inner1+","+fund(S.inner2,"θ("+fund(Term_dom_S_inner2.inner1,"0")+","+Term_fund_S_fund_T_0.inner2+")")+")"; //3.3.2.1
        else return "θ("+S.inner1+","+fund(S.inner2,"θ("+fund(Term_dom_S_inner2.inner1,"0")+",0)")+")"; //3.3.2.2
      }
    }
  }
  if (S instanceof PsiTerm){ //4
    ...
    if (equal(Term_dom_S_inner,"0")){ //4.1
      ...
      if (equal(Term_dom_S_sub,"0")||equal(Term_dom_S_sub,"θ(0,0)")) return T+""; //4.1.1
        ...
        if (lessThan(Term_dom_S_sub,S)) return "ψ("+fund(S.sub,T)+","+S.inner+")"; //4.1.3.1
          ...
          if (equal(dom(Term_dom_S_sub.sub),"θ(0,0)")){ //4.1.3.2.1
...
/**
 * @param {number} n 
 * @returns {string} ψ(0,ψ(θ(0,Λ(n)),0))
 */
function limitOrd(n){
  return "ψ(0,ψ(θ(0,"+"θ(".repeat(n+1)+"0"+",0)".repeat(n+1)+"),0))";
}
/**
 * @param {string} X 
 * @param {number} n 
