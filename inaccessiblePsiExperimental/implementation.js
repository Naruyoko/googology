...
Term.build=function (s,context){
  ...
  var TOP=0;
  var ITERMINNER=1;
  var ITERMSUBSCRIPT=2;
  var PSITERMSUBSCRIPT=3;
  var PSITERMINNER=4;
  var BRACES=5;
  var contextNames=["TOP","ITERMINNER","ITERMSUBSCRIPT","PSITERMSUBSCRIPT","PSITERMINNER","BRACES"];
  ...
  while (scanner.hasNext()&&state!=EXIT){
    ...
    if (symbolType==NUMBER){
      ...
      if (nextnext=="_"){
        var innerterm=Term.build(scanner,ITERMSUBSCRIPT);
        appendToRSum(ITerm.build(innerterm));
      }else{
        var innerterm=Term.build(scanner,ITERMINNER);
        var nextnext=scanner.next();
    ...
    if (state==CLOSEDTERM){
      ...
      if (context==BRACES&&peek=="}"){
        ...
        state=EXIT;
      }else if (context==ITERMINNER&&peek==")"){
        state=EXIT;
      }else if (context==ITERMSUBSCRIPT){
        state=EXIT;
      }
    }
  }
 ...
 * @returns {ITerm}
 */
ITerm.buildNoClone=function (sub){
  var r=new ITerm();
  r.sub=sub;
  return r;
}
...
ITerm.prototype.clone=function (){
  return ITerm.build(this.sub);
}
/** @param {boolean} abbreviate */
ITerm.prototype.toString=function (abbreviate){
  if (abbreviate&&(abbreviate===true||abbreviate["1I"])) return "I_"+this.sub.toStringWithImplicitBrace(abbreviate);
  return "I("+this.sub.toString(abbreviate)+")";
}
ITerm.prototype.equal=function (other){
  if (!(other instanceof Term)) other=new Term(other);
  return other instanceof ITerm&&this.sub.equal(other.sub);
}
...
function inT(t){
  ...
  if (t instanceof ITerm) return inTPsi(t.sub); //4
...
function lessThan(S,T){
  ...
  if (S instanceof ITerm){ //4
    ...
    if (T instanceof ITerm) return lessThan(S.sub,T.sub); //4.2
...
function dom(S){
  ...
  if (S instanceof ITerm){ //3
    ...
    if (equal(Term_dom_S_sub,Term.ZERO)||equal(Term_dom_S_sub,Term.ONE)) return S+""; //3.1
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)){ //4.1
      ...
      if (equal(Term_dom_S_sub,Term.ZERO)||equal(Term_dom_S_sub,Term.ONE)) return S+""; //4.1.1
        ...
        if (lessThan(Term_dom_S_sub,S)) return dom_S_sub; //4.1.2.1
          ...
          var InaDecons_S_sub=InaDecons(S.sub);
          if (InaDecons_S_sub==0) return Term.SMALLOMEGA+""; //4.1.2.2.1
          else return S+""; //4.1.2.2.2
        }
      }
 ...
 */
function Ina(S){
  return normalizeAbbreviations(InaDecons(S));
}
/**
 * @param {Term|string} S
 * @returns {number}
 */
function InaDecons(S){
  if (!(S instanceof Term)) S=new Term(S);
  if (!inT(S)) throw Error("Invalid argument: "+S);
  if (S instanceof ZeroTerm) return 0; //1
  if (S instanceof SumTerm) return InaDecons(S.getRight()); //2
  if (S instanceof ITerm){ //3
    var dom_S_sub=dom(S.sub);
    var Term_dom_S_sub=new Term(dom_S_sub);
    if (equal(Term_dom_S_sub,Term.ZERO)||equal(Term_dom_S_sub,Term.ONE)) return toNat(Term_dom_S_sub); //3.1
    else return InaDecons(S.sub); //3.2
  }
  if (S instanceof PsiTerm){ //4
    var dom_S_inner=dom(S.inner);
    var Term_dom_S_inner=new Term(dom_S_inner);
    if (equal(Term_dom_S_inner,Term.ZERO)){ //4.1
      var dom_S_sub=dom(S.sub);
      var Term_dom_S_sub=new Term(dom_S_sub);
      if (equal(Term_dom_S_sub,Term.ZERO)||equal(Term_dom_S_sub,Term.ONE)) return 0; //4.1.1
      else{ //4.1.2
        if (lessThan(Term_dom_S_sub,S)) return InaDecons(S.sub); //4.1.2.1
        else{ //4.1.2.2
          var InaDecons_S_sub=InaDecons(S.sub);
          if (InaDecons_S_sub==0) return 0; //4.1.2.2.1
          else return InaDecons_S_sub-1; //4.1.2.2.2
        }
      }
    }else if (equal(Term_dom_S_inner,Term.ONE)) return 0; //4.2
    else{ //4.3
      if (lessThan(Term_dom_S_inner,S)) return InaDecons(S.inner); //4.4.1
      else return 0; //4.4.2
    }
  }
  throw Error("No rule to compute Ina("+S+")");
}
/**
 * @param {Term|string} S 
...
function fund(S,T){
  ...
  if (S instanceof ITerm){ //3
    var dom_S_sub=dom(S.sub);
    var Term_dom_S_sub=new Term(dom_S_sub);
    if (equal(Term_dom_S_sub,Term.ZERO)){ //3.1
      if (equal(T,Term.ZERO)) return "ψ(0,0)"; //3.1.1
      else return "ψ("+T+",0)"; //3.1.2
    }else if (equal(Term_dom_S_sub,Term.ONE)){ //3.2
      if (equal(T,Term.ZERO)) return "I("+fund(S.sub,Term.ZERO)+")"; //3.2.1
      else return "I("+fund(S.sub,Term.ZERO)+")+ψ("+T+",0)"; //3.2.2
    }else return "I("+fund(S.sub,T)+")"; //3.3
  }
  if (S instanceof PsiTerm){ //4
    var dom_S_inner=dom(S.inner);
    var Term_dom_S_inner=new Term(dom_S_inner);
    if (equal(Term_dom_S_inner,Term.ZERO)){ //4.1
      var dom_S_sub=dom(S.sub);
      var Term_dom_S_sub=new Term(dom_S_sub);
      if (equal(Term_dom_S_sub,Term.ZERO)) return T+""; //4.1.1
      else if (equal(Term_dom_S_sub,Term.ONE)){ //4.1.2
        if (equal(T,Term.ZERO)) return "ψ("+fund(S.sub,Term.ZERO)+","+S.inner+")"; //4.1.2.1
        else return "ψ("+fund(S.sub,Term.ZERO)+","+T+")"; //4.1.2.2
      }else{ //4.1.3
        if (lessThan(Term_dom_S_sub,S)) return "ψ("+fund(S.sub,T)+","+S.inner+")"; //4.1.3.1
        else{ //4.1.3.2
          if (InaDecons(S.sub)==0){ //4.1.3.2.1
            var Term_fund_S_fund_T_0=null;
            if (notEqual(T,Term.ZERO)&&isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.inner,S.inner)) return "ψ("+fund(S.sub,Term_fund_S_fund_T_0.sub)+","+S.inner+")"; //4.1.3.2.1.1
            else return "ψ("+fund(S.sub,Term.ZERO)+","+S.inner+")"; //4.1.3.2.1.2
          }else return "ψ("+fund(S.sub,T)+","+S.inner+")"; //4.1.3.2.2
        }
      }
      ...
      if (lessThan(Term_dom_S_inner,S)) return "ψ("+S.sub+","+fund(S.inner,T)+")"; //4.3.1
        ...
        var Term_fund_S_fund_T_0=null;
        if (notEqual(T,Term.ZERO)&&isNat(T)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,Term_fund_S_fund_T_0.inner)+")"; //4.3.2.1
        else return "ψ("+S.sub+","+fund(S.inner,Term.ZERO)+")"; //4.3.2.2
      }
    }
  }
 ...
 */
function limitOrd(n){
  return "ψ(0,ψ("+"I(ψ(".repeat(n+1)+"0"+",0))".repeat(n+1)+",0))";
}
/**
 * @param {number} n 
 * @returns {number}
...
var testTermsPre=[
  ...
  ["ψ_0(ψ_ψ_ω(0)(0))",3],
  ["ψ_0(ψ_I_0(0))",3],
  ["ψ_0(ψ_I_0(0)+ψ_I_0(0))",3],
  ["ψ_0(ψ_I_0(1))",-1],
  ["ψ_0(ψ_I_0(ψ_I_0(0)))",3],
  ["ψ_0(ψ_I_0+1(0))",3],
  ["ψ_0(ψ_I_0+ψ_ψ_I_0(0)(0)(0))",3],
  ["ψ_0(ψ_I_0+ψ_ψ_I_0(0)+1(0)(0))",-1],
  ["ψ_0(ψ_I_0+ψ_ψ_I_0+1(0)(0)(0))",3],
  ["ψ_0(ψ_I_0+ψ_ψ_I_0+2(0)(0)(0))",-1],
  ["ψ_0(ψ_I_0+ψ_ψ_I_0+2(0)+ψ_I_0+1(ψ_I_0+2(0))(0)(0))",3],
  ["ψ_0(ψ_I_0+ψ_ψ_I_0+2(0)+ψ_I_0+2(0)(0)(0))",3],
  ["ψ_0(ψ_I_1(0))",3],
  ["ψ_0(ψ_I_1(0)+ψ_I_0(0))",-1],
  ["ψ_0(ψ_I_1(0)+ψ_I_0(ψ_I_1(0)))",3],
  ["ψ_0(ψ_I_1(0)+ψ_I_0+1(0))",3],
  ["ψ_0(ψ_I_1(0)+ψ_I_0+ψ_ψ_I_1(0)(0)(0))",3],
  ["ψ_0(ψ_I_1(0)+ψ_I_1(0))",3],
  ["ψ_0(ψ_I_1(ψ_I_0(0)))",-1],
  ["ψ_0(ψ_I_1(ψ_I_0(ψ_I_1(0))))",3],
  ["ψ_0(ψ_I_1(ψ_I_0+1(0)))",3],
  ["ψ_0(ψ_I_1(ψ_I_0+ψ_ψ_I_0(0)(0)(0)))",-1],
  ["ψ_0(ψ_I_1(ψ_I_0+ψ_ψ_I_0+1(0)(0)(0)))",-1],
  ["ψ_0(ψ_I_1(ψ_I_0+ψ_ψ_I_1(0)(0)(0)))",3],
  ["ψ_0(ψ_I_1(ψ_I_1(0)))",3],
  ["ψ_0(ψ_I_1+1(0))",3],
  ["ψ_0(ψ_I_1+ψ_ψ_I_0(0)(0)(0))",3],
  ["ψ_0(ψ_I_1+ψ_ψ_I_0+1(0)(0)(0))",3],
  ["ψ_0(ψ_I_1+ψ_ψ_I_1(0)(0)(0))",3],
  ["ψ_0(ψ_I_1+ψ_ψ_I_1(0)+ψ_I_1(0)(0)(0))",3],
  ["ψ_0(ψ_I_1+ψ_ψ_I_1(1)(0)(0))",-1],
  ["ψ_0(ψ_I_1+ψ_ψ_I_1+1(0)(0)(0))",3],
  ["ψ_0(ψ_I_2(0))",3],
  ["ψ_0(ψ_I_2+1(0))",3],
  ["ψ_0(ψ_I_2+ψ_ψ_I_2(0)(0)(0))",3],
  ["ψ_0(ψ_I_2+ψ_ψ_I_2(0)+ψ_I_2(0)(0)(0))",3],
  ["ψ_0(ψ_I_2+ψ_ψ_I_2(1)(0)(0))",-1],
  ["ψ_0(ψ_I_2+ψ_ψ_I_2+1(0)(0)(0))",3],
  ["ψ_0(ψ_I_3(0))",3],
  ["ψ_0(ψ_I_ω(0))",3],
  ["ψ_0(ψ_I_ω+ψ_ψ_I_ω(0)(0)(0))",3],
  ["ψ_0(ψ_I_{ω+1}(0))",3],
  ["ψ_0(ψ_I_{ω+1}+ψ_ψ_I_{ω+1}(0)(0)(0))",3],
  ["ψ_0(ψ_I_ψ_1(0)(0))",3],
  ["ψ_0(ψ_I_ψ_I_0(0)(0))",3]
];
/** @type {string[]}} */
