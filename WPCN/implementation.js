  ...
  if (S instanceof SumTerm){ //2
    ...
    if (equal(Term_cp_b,Term.ONE)) return cut(S); //2.1
      ...
      if (lessThan(Term_br_b,Term_cp_b)) return a+"+"+fund(b,T); //2.2.1
        ...
        if (lessThanOrEqual(Term_cp_b,a)){ //2.2.2.1
          if (T==0) return cut(S)+"+"+fund(cp(b),0); //2.2.2.1.1
          else return cut(S)+"+"+fund(cp(b),0)+"+"+fund(S,T-1); //2.2.2.1.2
        }else return a+"+"+fund(b,T); //2.2.2.2
      }
    }
  }
  if (S instanceof PrincipalTerm){ //3
    if (equal(S.inner,Term.ZERO)) return "0"; //3.1
    else{ //3.2
      if (T==0) return S.inner+""; //3.2.1
      else return S.inner+"+"+fund(S,T-1); //3.2.2
    }
  }
 ...
 */
function limitOrd(n){
  return "(".repeat(n+1)+"0"+")".repeat(n+1);
}
