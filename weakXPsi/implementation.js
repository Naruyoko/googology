...
Term.build=function (s,context){
  ...
  var PSITERMINNER=3;
  var BRACES=4;
  var contextNames=["TOP","ITERMINNER","PSITERMSUBSCRIPT","PSITERMINNER","BRACES"];
  ...
  while (scanner.hasNext()&&state!=EXIT){
    ...
    if (symbolType==NUMBER){
      ...
      var innerterm=Term.build(scanner,XTERMINNER);
      var nextnext=scanner.next();
      if (nextnext!=")") throw Error("Expected closing ) at position "+(scanner.pos-1)+", instead got "+nextnext+" in expression "+scanner.s);
      appendToRSum(XTerm.build(innerterm));
    ...
    if (state==CLOSEDTERM){
      ...
      if (context==BRACES&&peek=="}"){
        ...
        state=EXIT;
      }else if (context==XTERMINNER&&peek==")"){
        state=EXIT;
      }
    }
  }
 ...
 */
function XTerm(s){
  if (s instanceof XTerm) return s.clone();
  else if (s instanceof Term&&typeof s!="string") throw Error("Invalid expression: "+s);
  if (!(this instanceof XTerm)) return new XTerm(s);
  /** @type {XTerm} */
  Term.call(this,s);
  if (s&&!(this instanceof XTerm)) throw Error("Invalid expression: "+s);
  /** @type {Term} */
  this.inner=null;
}
Object.assign(XTerm,Term);
XTerm.build=function (inner){
  var r=new XTerm();
 ...
 * @returns {XTerm}
 */
XTerm.buildNoClone=function (inner){
  var r=new XTerm();
  r.inner=inner;
  return r;
}
XTerm.prototype=Object.create(Term.prototype);
XTerm.prototype.clone=function (){
  return XTerm.build(this.inner);
}
/** @param {boolean} abbreviate */
XTerm.prototype.toString=function (abbreviate){
  return "x("+this.inner.toString(abbreviate)+")";
}
XTerm.prototype.equal=function (other){
  if (!(other instanceof Term)) other=new Term(other);
  return other instanceof XTerm&&this.inner.equal(other.inner);
}
Object.defineProperty(XTerm.prototype,"constructor",{
  value:XTerm,
  enumerable:false,
  writable:true
});
...
function inT(t){
  ...
  if (t instanceof XTerm) return inTPsi(t.inner); //4
...
function lessThan(S,T){
  ...
  if (S instanceof SumTerm){ //3
    ...
    if (T instanceof XTerm) return lessThan(S.getLeft(),T); //3.2
    if (T instanceof PsiTerm) return lessThan(S.getLeft(),T); //3.3
  }
  if (S instanceof XTerm){ //4
    if (T instanceof SumTerm) return lessThanOrEqual(S,T.getLeft()); //4.1
    if (T instanceof XTerm) return lessThan(S.inner,T.inner); //4.2
    if (T instanceof PsiTerm) return notEqual(T.sub,Term.ZERO)||notEqual(T.inner,Term.ZERO); //4.3
  }
  if (S instanceof PsiTerm){ //5
    if (T instanceof SumTerm) return lessThanOrEqual(S,T.getLeft()); //5.1
    if (T instanceof XTerm) return equal(S.sub,Term.ZERO)&&equal(S.inner,Term.ZERO); //5.2
...
function dom(S){
  ...
  if (S instanceof PsiTerm){ //4
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)){ //4.1
      ...
      if (equal(Term_dom_S_sub,Term.ZERO)||equal(Term_dom_S_sub,Term.ONE)||equal(Term_dom_S_sub,Term.XZERO)) return S+""; //4.1.1
      else return dom_S_sub; //4.1.2
...
function fund(S,T){
  ...
  if (S instanceof XTerm){ //3
    ...
    if (equal(Term_dom_S_inner,Term.ZERO)) return T+""; //3.1
      ...
      else return "x("+fund(S.inner,Term.ZERO)+")"; //3.2.2
    }else return "x("+fund(S.inner,T)+")"; //3.3
  }
  if (S instanceof PsiTerm){ //4
    var dom_S_inner=dom(S.inner);
    var Term_dom_S_inner=new Term(dom_S_inner);
    if (equal(Term_dom_S_inner,Term.ZERO)){ //4.1
      var dom_S_sub=dom(S.sub);
      var Term_dom_S_sub=new Term(dom_S_sub);
      if (equal(Term_dom_S_sub,Term.ZERO)||equal(Term_dom_S_sub,Term.ONE)||equal(Term_dom_S_sub,Term.XZERO)) return T+""; //4.1.1
      else return "ψ("+fund(S.sub,T)+","+S.inner+")"; //4.1.2
      ...
      if (lessThan(Term_dom_S_inner,S)) return "ψ("+S.sub+","+fund(S.inner,T)+")"; //4.3.1
        ...
        if (equal(Term_dom_c,Term.ONE)){ //4.3.2.1
          ...
          if (equal(dom(S.sub),Term.XZERO)){ //4.3.2.2.1
            var Term_fund_S_fund_T_0=null;
            if (equal(dom(T),Term.ONE)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"ψ("+fund(c,Term.ZERO)+","+Term_fund_S_fund_T_0.inner+")")+")"; //4.3.2.2.1.1
            else return "ψ("+S.sub+","+fund(S.inner,"ψ("+fund(c,Term.ZERO)+",0)")+")"; //4.3.2.2.1.2
          }else{ //4.3.2.2.2
            var Term_fund_S_fund_T_0=null;
            if (equal(dom(T),Term.ONE)&&(Term_fund_S_fund_T_0=new Term(fund(S,fund(T,Term.ZERO)))) instanceof PsiTerm&&equal(Term_fund_S_fund_T_0.sub,S.sub)) return "ψ("+S.sub+","+fund(S.inner,"ψ("+fund(c,S.sub)+","+Term_fund_S_fund_T_0.inner+")")+")"; //4.3.2.2.2.1
            else return "ψ("+S.sub+","+fund(S.inner,"ψ("+fund(c,S.sub)+",0)")+")"; //4.3.2.2.2.2
          }
        }
      }
    }
 ...
 */
function limitOrd(n){
  return "ψ(0,"+("ψ("+Term.SMALLOMEGA+",ψ(x(").repeat(n+1)+"0"+"),0))".repeat(n+1)+")";
}
/**
 * @param {number} n 
 * @returns {number}
...
var testTermsPre=[
  ...
  ["ψ_0(ψ_ω(ψ_0(ψ_ω(0))))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(0)))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(0))+ψ_0(ψ_ω(ψ_x(0)(0))))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(0))+ψ_1(0))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(0))+ψ_1(ψ_ω(ψ_x(0)(0))))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(0))+ψ_2(0))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(0))+ψ_ω(0))",-1],
  ["ψ_0(ψ_ω(ψ_x(0)(0))+ψ_ω(ψ_x(0)(0)))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(0)+1))",-1],
  ["ψ_0(ψ_ω(ψ_x(0)(0)+ψ_x(0)(0)))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(1)))",-1],
  ["ψ_0(ψ_ω(ψ_x(0)(ψ_0(ψ_ω(ψ_x(0)(0))))))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(ψ_1(0))))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(ψ_1(ψ_ω(ψ_x(0)(0))))))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(ψ_2(0))))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(ψ_ω(0))))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(ψ_ω(ψ_0(ψ_ω(ψ_x(0)(0)))))))",3],
  ["ψ_0(ψ_ω(ψ_x(0)(ψ_ω(ψ_x(0)(0)))))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0)))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0))+ψ_ω(ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0)))))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0))+ψ_ω(ψ_x(0)(0)))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0))+ψ_ω(ψ_x(0)(ψ_ω(ψ_x(0)(0)))))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0))+ψ_ω(ψ_x(0)(ψ_ω(ψ_{x(0)+x(0)}(0)))))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0))+ψ_ω(ψ_{x(0)+x(0)}(0)))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0)+1))",-1],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0)+ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0)))))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0)+ψ_x(0)(0)))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0)+ψ_x(0)(ψ_ω(ψ_x(0)(0)))))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0)+ψ_x(0)(ψ_ω(ψ_{x(0)+x(0)}(0)))))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(0)+ψ_{x(0)+x(0)}(0)))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(1)))",-1],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(ψ_ω(ψ_x(0)(0)))))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(ψ_ω(ψ_x(0)(ψ_ω(ψ_{x(0)+x(0)}(0)))))))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)}(ψ_ω(ψ_{x(0)+x(0)}(0)))))",3],
  ["ψ_0(ψ_ω(ψ_{x(0)+x(0)+x(0)}(0)))",3],
  ["ψ_0(ψ_ω(ψ_x(1)(0)))",-1],
  ["ψ_0(ψ_ω(ψ_x(1)(ψ_ω(ψ_x(0)(0)))))",3],
  ["ψ_0(ψ_ω(ψ_{x(1)+x(0)}(0)))",-1],
  ["ψ_0(ψ_ω(ψ_{x(1)+x(0)}(ψ_ω(ψ_x(0)(0)))))",3],
  ["ψ_0(ψ_ω(ψ_{x(1)+x(0)}(ψ_ω(ψ_x(1)(0)))))",-1],
  ["ψ_0(ψ_ω(ψ_{x(1)+x(0)}(ψ_ω(ψ_{x(1)+x(0)}(0)))))",3],
  ["ψ_0(ψ_ω(ψ_{x(1)+x(1)}(0)))",-1],
  ["ψ_0(ψ_ω(ψ_x(2)(0)))",-1],
  ["ψ_0(ψ_ω(ψ_x(ψ_0(ψ_ω(ψ_x(0)(0))))(0)))",3],
  ["ψ_0(ψ_ω(ψ_x(ψ_1(0))(0)))",3],
  ["ψ_0(ψ_ω(ψ_x(ψ_1(ψ_ω(ψ_x(0)(0))))(0)))",3],
  ["ψ_0(ψ_ω(ψ_x(ψ_2(0))(0)))",-1],
  ["ψ_0(ψ_ω(ψ_x(ψ_ω(0))(0)))",-1],
  ["ψ_0(ψ_ω(ψ_x(ψ_ω(ψ_x(0)(0)))(0)))",3],
  ["ψ_0(ψ_ω(ψ_x(ψ_ω(ψ_{x(0)+x(0)}(0)))(0)))",3],
  ["ψ_0(ψ_ω(ψ_x(ψ_ω(ψ_x(1)(0)))(0)))",-1],
  ["ψ_0(ψ_ω(ψ_x(ψ_ω(ψ_x(ψ_ω(ψ_x(0)(0)))(0)))(0)))",-1],
];
/** @type {string[]}} */
var testTerms;
